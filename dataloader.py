...
def read_corpus(path, is_bert=False,clean=True, MR=True, encoding='utf8', shuffle=False, lower=True):
    data = []
    labels = []
    with open(path, encoding=encoding) as fin:
        if "mnli" in path:
            for line in fin:
                ...
                if clean:
                    text_a = clean_str(text_a.strip()) if clean else text_a.strip()
                    text_b = clean_str(text_b.strip()) if clean else text_b.strip()
                if lower:
                    text_a = text_a.lower()
                    text_b = text_b.lower()

                if not is_bert:
                    text = text_a + " </s> " + text_b
                else:
