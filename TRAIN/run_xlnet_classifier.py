...
# from pytorch_pretrained_bert.optimization import WarmupLinearSchedule as warmup_linear
...
class IMDBProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_corpus(os.path.join(data_dir, "train"), MR=True, clean=False, shuffle=True), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_corpus(os.path.join(data_dir, "test"), MR=True, clean=False, shuffle=True), "dev")
...
import os
os.environ["http_proxy"] = "http://172.19.122.129:7890"
os.environ["https_proxy"] = "http://172.19.122.129:7890"
...
def convert_examples_to_features_mnli(examples, label_list, max_seq_length, tokenizer):
...
def main():
    ...
    tokenizer = XLNetTokenizer.from_pretrained(args.bert_model, do_lower_case=args.do_lower_case)
    ...
    model = XLNetForSequenceClassification.from_pretrained(args.bert_model,
                                                          num_labels=num_labels)
    ...
    if args.do_train:
        if task_name == "mnli":
            train_features = convert_examples_to_features_mnli(
    ...
    if args.do_train:
        ...
        model = XLNetForSequenceClassification.from_pretrained(config, num_labels=num_labels)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        if task_name == "mnli":
            eval_features = convert_examples_to_features_mnli(
